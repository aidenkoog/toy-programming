...
class SerializeConvertAdapter<T> private constructor(
    type: Type, private val kSerializer: KSerializer<T>
) : Converter<T> {
    private fun SerialFormat.serializeAsType(type: Type): KSerializer<Any> =
    ...
    private val json = Json {
        ...
        coerceInputValues =
            true // Set as non-null type but receive as null type. you can use default value as set true
        encodeDefaults =
            true // You can use default value when received data does not have that value.
    }

    private val stringFormat: StringFormat = json.apply {
        ignoreIoExceptions {
            webSocketLog("[IoException] Cause convert specific type is not supported.")
        }
    }

    private val loader: DeserializationStrategy<T> =
        stringFormat.serializeAsType(type) as DeserializationStrategy<T>
